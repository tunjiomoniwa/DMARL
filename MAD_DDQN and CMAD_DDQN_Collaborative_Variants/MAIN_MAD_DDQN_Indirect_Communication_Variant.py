...
### step files
from RWP import RWP  ## This is for the random waypoint
...
##        syn_data_y = random.sample(range(40, 960), 200-194)
        ...
        for i in range(n_games):
            ...
            s1 = np.array([x1, y1, z1, 100, 100])
            s2 = np.array([x2, y2, z2, 100, 100])
            s3 = np.array([x3, y3, z3, 100, 100])
            s4 = np.array([x4, y4, z4, 100, 100])
             

            sprime1val = np.array([x1, y1, z1, 100, 100])
            sprime2val = np.array([x2, y2, z2, 100, 100])
            sprime3val = np.array([x3, y3, z3, 100, 100])
            sprime4val = np.array([x4, y4, z4, 100, 100])
            ...
            while itern<Max_iterations:# not done1:
               ...
               ############### 
                ...
                sprime1val = np.array([sprime1[0], sprime1[1], sprime1[2], cov_obs[0]-old_cov1, sprime1[3]/(sprime1[3]+sprime1[3])])
                sprime2val = np.array([sprime2[0], sprime2[1], sprime2[2], cov_obs[1]-old_cov2, sprime2[3]/(sprime2[3]+sprime2[3])])
                sprime3val = np.array([sprime3[0], sprime3[1], sprime3[2], cov_obs[2]-old_cov3, sprime3[3]/(sprime3[3]+sprime3[3])])
...
##                 
            ...
            np.savetxt('results_test/Energy_mad.dat', enetot)
            #np.savetxt('results_test/Reward_mad.dat', r_sum1, r_sum2, r_sum3, r_sum4)    
            np.savetxt('results_test/Coverage_mad.dat', covtot)
            np.savetxt('results_test/Fair_mad.dat', fairtot)
            np.savetxt('results_test/ee_mad.dat', efficiency)
