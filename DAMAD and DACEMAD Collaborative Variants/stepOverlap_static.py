...
def stepOverlap_static(xo1, yo1, zo1, xo2, yo2, zo2,xo3, yo3, zo3, xo4, yo4, zo4, xsef, ysef):
    ...
    r1_ = pow((cov_rad*zo1), 2) - pow(zo1,2)
    r1 = pow(r1_, 0.5)
    tl = np.cos(th)
    ...
    #finx  =  np.where(xsef==tot_deviceID) #xsef(tot_deviceID)
    #print(finx)

    #lenp = np.array(range(0,len(tot_deviceID)))

    #print(len(tot_deviceID))


    #xsef = np.array(xsef)
    #finx = np.where(xsef==tot_deviceID)


    #finx  = xsef.index(tot_deviceID)


    #print(finx)

    #finx = xsef[lenp]
    #finy = ysef[range(len(tot_deviceID))]
    ...
    return [connectionCount_uav1, connectionCount_uav2, connectionCount_uav3, connectionCount_uav4]

    #print([connectionCount_uav1, connectionCount_uav2, connectionCount_uav3, connectionCount_uav4])
