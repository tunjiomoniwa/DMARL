...
##        print('Loading sumo high traffic data at time',i)
    ...
    data_bank_low = []
    for i in range(18005, Max_iterations+18006):
    ...
    xsef = cc[0] #np.concatenate((xsef_mob, xblock1, xblock2, xblock3, xblock4))
    ysef = cc[1] #np.concatenate((ysef_mob, yblock1, yblock2, yblock3, yblock4))


    
    
    x1 =  random.randint(250, 490)
    ...
    y2 = random.randint(300, 600)
    z2 = 110
    e2 = 0
    dist2 = 0 
    x3 =  random.randint(200, 450)
    y3 =  random.randint(300, 550)
    z3 = 110
    e3 = 0
    dist3 = 0 
    x4 =  random.randint(200, 400)
    ...
    dist7 = 0
    ##
    x8 =  random.randint(400, 650)
    y8 =  random.randint(300, 600)
    z8 = 110
    e8 = 0
    dist8 = 0

    x9 =  random.randint(400, 700)
    y9 =  random.randint(600, 800)
    z9 = 110
    e9 = 0
    dist9 = 0

    x10 =  random.randint(600, 800)
    y10 =  random.randint(550, 800)
    z10 = 110
    e10 = 0
    dist10 = 0
...
##    dense_goal10 = x10>400 and x10<700 and y10>300 and y10<600
    ...
    for i in range(n_games):
        ...
        sprime5val = np.array([x5, y5, z5, 100, 100, 100, 5, 5, 5, 5, 0, 0, 0, 0])
        sprime6val = np.array([x6, y6, z6, 100, 100, 100, 5, 5, 5, 5, 0, 0, 0, 0])
        sprime7val = np.array([x7, y7, z7, 100, 100, 100, 5, 5, 5, 5, 0, 0, 0, 0])
        sprime8val = np.array([x8, y8, z8, 100, 100, 100, 5, 5, 5, 5, 0, 0, 0, 0])
        sprime9val = np.array([x9, y9, z9, 100, 100, 100, 5, 5, 5, 5, 0, 0, 0, 0])
        sprime10val = np.array([x10, y10, z10, 100, 100, 100, 5, 5, 5, 5, 0, 0, 0, 0])
...
##        
        ...
        while itern<Max_iterations:# not done1:
           ...
           ############### 
            ...
            tot_tp = tp_sum+(reached_tp*(1600-itern))
            ...
            sprime2val = np.array([sprime2[0], sprime2[1], sprime2[2], N_2a, N_2b, N_2c, cov_obs[1]-old_cov2, cd2a, cd2b, cd2c, sprime2[3]/(sprime2[3]+sprime2[3]), ed2a, ed2b, ed2c, x2_target, y2_target, ((cov_obs[1]+1)/(past_cov2+1))])
            sprime3val = np.array([sprime3[0], sprime3[1], sprime3[2], N_3a, N_3b, N_3c, cov_obs[2]-old_cov3, cd3a, cd3b, cd3c, sprime3[3]/(sprime3[3]+sprime3[3]), ed3a, ed3b, ed3c, x3_target, y3_target, ((cov_obs[2]+1)/(past_cov3+1))])
            sprime4val = np.array([sprime4[0], sprime4[1], sprime4[2], N_4a, N_4b, N_4c, cov_obs[3]-old_cov4, cd4a, cd4b, cd4c, sprime4[3]/(sprime4[3]+sprime4[3]), ed4a, ed4b, ed4c, x4_target, y4_target, ((cov_obs[3]+1)/(past_cov4+1))])
            sprime5val = np.array([sprime5[0], sprime5[1], sprime5[2], N_5a, N_5b, N_5c, cov_obs[4]-old_cov5, cd5a, cd5b, cd5c, sprime5[3]/(sprime5[3]+sprime5[3]), ed5a, ed5b, ed5c, x5_target, y5_target, ((cov_obs[4]+1)/(past_cov5+1))])
            sprime6val = np.array([sprime6[0], sprime6[1], sprime6[2], N_6a, N_6b, N_6c, cov_obs[5]-old_cov6, cd6a, cd6b, cd6c, sprime6[3]/(sprime6[3]+sprime6[3]), ed6a, ed6b, ed6c, x6_target, y6_target, ((cov_obs[5]+1)/(past_cov6+1))])
            sprime7val = np.array([sprime7[0], sprime7[1], sprime7[2], N_7a, N_7b, N_7c, cov_obs[6]-old_cov7, cd7a, cd7b, cd7c, sprime7[3]/(sprime7[3]+sprime7[3]), ed7a, ed7b, ed7c, x7_target, y7_target, ((cov_obs[6]+1)/(past_cov7+1))])
            sprime8val = np.array([sprime8[0], sprime8[1], sprime8[2], N_8a, N_8b, N_8c, cov_obs[7]-old_cov8, cd8a, cd8b, cd8c, sprime8[3]/(sprime8[3]+sprime8[3]), ed8a, ed8b, ed8c, x8_target, y8_target, ((cov_obs[7]+1)/(past_cov8+1))])
            sprime9val = np.array([sprime9[0], sprime9[1], sprime9[2], N_9a, N_9b, N_9c, cov_obs[8]-old_cov9, cd9a, cd9b, cd9c, sprime9[3]/(sprime9[3]+sprime9[3]), ed9a, ed9b, ed9c, x9_target, y9_target, ((cov_obs[8]+1)/(past_cov9+1))])
            sprime10val = np.array([sprime10[0], sprime10[1], sprime10[2], N_10a, N_10b, N_10c, cov_obs[9]-old_cov10, cd10a, cd10b, cd10c, sprime10[3]/(sprime10[3]+sprime10[3]), ed10a, ed10b, ed10c, x10_target, y10_target, ((cov_obs[9]+1)/(past_cov10+1))])
