...
deployed_veh_dacemad = dlmread('deployed_vehicles_m50_congested_dacemad.dat');
covered_veh_dacemad = sum(dlmread('Covered_vehicles_m50_congested_dacemad.dat'));
deployed_veh_damad = dlmread('deployed_vehicles_m50_congested_damad.dat');
covered_veh_damad = sum(dlmread('Covered_vehicles_m50_congested_damad.dat'));
deployed_veh_hi = dlmread('deployed_vehicles_box_test_h.dat');
covered_veh_hi = sum(dlmread('Covered_vehicles_box_test_h.dat'));

covered_veh_mad = sum(dlmread('Covered_vehicles_M50_congested_mad.dat'));
deployed_veh_mad = dlmread('deployed_vehicles_M50_congested_mad.dat');
covered_veh_cmad = sum(dlmread('Covered_vehicles_M50_congested_cmad.dat'));
deployed_veh_cmad = dlmread('deployed_vehicles_M50_congested_cmad.dat');
...
if (plotCDR == 1)
    ...
    axis([1 length(deployed_veh) 0 280])
    grid on;    
end
