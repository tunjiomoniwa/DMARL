...
deployed_veh_dacemad = dlmread('deployed_vehicles_box_test_4.dat');
covered_veh_dacemad = sum(dlmread('Covered_vehicles_box_test_4.dat'));
deployed_veh_damad = dlmread('deployed_vehicles_DEMAD_2.dat');
covered_veh_damad = sum(dlmread('Covered_vehicles_DEMAD_2.dat'));
deployed_veh_hi = dlmread('deployed_vehicles_box_test_h.dat');
covered_veh_hi = sum(dlmread('Covered_vehicles_box_test_h.dat'));

covered_veh_mad = sum(dlmread('Covered_vehicles_M50_MAD3.dat'));
deployed_veh_mad = dlmread('deployed_vehicles_M50_MAD3.dat');
covered_veh_cmad = sum(dlmread('Covered_vehicles_M50_CMAD2.dat'));
deployed_veh_cmad = dlmread('deployed_vehicles_M50_CMAD2.dat');
...
if (plotCDR == 1)
    ...
    grid on;
    a2 = axes();
    a2.Position = [0.590 0.3800 0.25 0.2]; 
