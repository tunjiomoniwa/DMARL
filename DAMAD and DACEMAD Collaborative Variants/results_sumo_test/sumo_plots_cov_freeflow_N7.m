...
deployed_veh_dacemad = dlmread('deployed_vehicles_N7_freeflow_dacemad.dat');
covered_veh_dacemad = sum(dlmread('Covered_vehicles_N7_freeflow_dacemad.dat'));
deployed_veh_damad = dlmread('deployed_vehicles_N7_freeflow_damad.dat');
covered_veh_damad = sum(dlmread('Covered_vehicles_N7_freeflow_damad.dat'));
deployed_veh_hi = dlmread('deployed_vehicles_box_test_h.dat');
covered_veh_hi = sum(dlmread('Covered_vehicles_box_test_h.dat'));

covered_veh_mad = sum(dlmread('Covered_vehicles_N7_freeflow_mad.dat'));
deployed_veh_mad = dlmread('deployed_vehicles_N7_freeflow_mad.dat');
covered_veh_cmad = sum(dlmread('Covered_vehicles_N7_freeflow_cmad.dat'));
deployed_veh_cmad = dlmread('deployed_vehicles_N7_freeflow_cmad.dat');
...
if (plotCDR == 1)
    ...
    axis([1 length(deployed_veh_dacemad) -0.05 1.3])    
    grid on;
    %a2 = axes();
    ...
    %plot(epi_lo, deployed_veh_dacemad, 'b-', epi_lo, covered_veh_dacemad, 'r-', 'Linewidth', 1)
    plot(epi_lo, deployed_veh_mad, 'b-', epi_lo, covered_veh_mad, 'r-', 'Linewidth', 1)
    ...
    legend('Deployed (MAD-DDQN)','Covered (MAD-DDQN)', 'location', 'best')
    %axis([000 500 200.0 700])    
    axis([1 length(deployed_veh) -2 130])
    grid on;    
end
