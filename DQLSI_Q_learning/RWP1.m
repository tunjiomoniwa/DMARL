function [finx, finy] = RWP1(mob_x, mob_y);
%%[newx, newy] = RWP(mob_x, mob_y);
    ...
    finy = [];
    for i = 1: num_devices
        [inc_x, inc_y] = circle(randi([-5 5]), randi([-2 3]), randi([0 8]), randi([0 180]), rand); 
        %%circle(randi([-5 5]), randi([-2 3]), randi([0 8]), randi([0 300]), rand);
