function obsUAVBL1 = stepBL1_static(action, x1, y1, z1, e1, overlap1, xo, yo, zo, xse, yse, xme1, yme1)
...
%     matyMin = [100 0 0 550 550 550];
    matxMax = [500 600 900 250 600 900];
    matxMin = [100 300 650 0 350 650];
    matyMax = [500 450 450 900 900 900];
    matyMin = [100 0 0 550 550 550];
    zMax = 400;
    zMin = 200;
...
%     DJI Phantom 4 Pro – 45 mph
    ...
    altitudestep = 15;
      ...
      for jj1 =1:numMobileEndDevices
       ...
       goal1 = logical(energy1 <= 300000 && z1 >= 400 && x1>=150  && x1<250  && y1>=200 && y1<300);
       
       %done1 = logical(energy1 <= 20001 && z1 >= 400 && x1>=230  && x1<280  && y1>=230 && y1<280); %border distribution
       done1 = logical(energy1 <= 300000 && z1 >= 390 && x1>=200  && x1<300  && y1>=200 && y1<300);  %random and uniform distribution
       ...
       obsUAVBL1 = [[x1,y1,z1], [energy1, coveragefactorstatic1, geog_area1], done1, dead1, goal1, uav1distFromgoal, coveragefactormobile, xme1, yme1, overlap1];
